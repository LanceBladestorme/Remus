exports.run = (client, message, args) => {
  ...
  message.channel.send(':zzz: Entering sleep mode! :zzz:').then(message.delete(10)).then(m => m.delete(5000)) && message.guild.members.find(x => x.id === client.user.id).setNickname('[Sleep Mode] Remus')
}
if (message.guild.members.find(x => x.id === client.user.id).roles.find(x => x.name === 'sleep mode')) {
...
message.channel.send(':dog: No longer in sleep mode! :dog:').then(message.delete(10)).then(m => m.delete(5000)) && message.guild.members.find(x => x.id === client.user.id).setNickname(client.user.username)
}
}
exports.help = {
  name: 'Sleep Mode',
  description: 'This command toggles sleep mode. While in sleep mode, the bot will not respond to commands.',
